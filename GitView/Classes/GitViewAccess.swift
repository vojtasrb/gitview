...
class GitViewAccess {
    ...
    func getCompleteInfo(username: String, repo: String, completionBlock: @escaping ([[String]]) -> Void) {
        
        // get branches
        AF.request("https://api.github.com/repos/\(username)/\(repo)/branches", method: .get).validate().responseJSON { response in
            switch response.result {
            case .success(let value):
                let json = JSON(value)
            ...
            // get commits
            AF.request("https://api.github.com/repos/\(username)/\(repo)/commits", method: .get).validate().responseJSON { response in
                switch response.result {
                case .success(let value):
                    let json = JSON(value)
